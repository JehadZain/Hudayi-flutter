        ...
        ? ProfilePage(
            tabs: jsonDecode(authService.user.toUser())["role"] == "student"
                ...
                : jsonDecode(authService.user.toUser())["role"] == "admin" ||
                    ...
                    : getStudentMainTabs(context),
            pageType: jsonDecode(authService.user.toUser())["role"] == "admin" ||
                    jsonDecode(authService.user.toUser())["role"] == "branch_admin" ||
                    jsonDecode(authService.user.toUser())["role"] == "property_admin"
                ? "admin"
              ...
              child: Stack(
                children: [
                  ...
                  Padding(
                    ...
                    child: Column(
                      mainAxisAlignment: MediaQuery.of(context).viewInsets.bottom > 0.0 ? MainAxisAlignment.start : MainAxisAlignment.center,
                      children: [
                        AnimatedCrossFade(
                            duration: const Duration(milliseconds: 300),
                            crossFadeState: MediaQuery.of(context).viewInsets.bottom > 0.0 ? CrossFadeState.showFirst : CrossFadeState.showSecond,
                            firstChild: const SizedBox(
                              height: 120,
                              width: 120,
                            ),
                            secondChild: MediaQuery.of(context).viewInsets.bottom > 0.0
                                ? Container()
                                : Image.asset(AppConstants.appLogo, height: 100, width: 100)),
                        Helper.sizedBoxH20,
                        StringTextField(
                        ...
                        StringTextField(
                            controller: passwordController, title: translate(context).password, readOnly: false, isRequired: true, obscureText: true),
                        Helper.sizedBoxH20,
                        !isConnected
                            ? const NoInternet()
                            : Center(
                                    ...
                                    onTap: () async {
                                      ...
                                      if (userNameController.text != "" && passwordController.text != "") {
                                        if (isConnected) {
                                          ...
                                          Map result = await ApiService().login(userNameController.text, passwordController.text);
                                          ...
                                          if (status == "") {
                                            ...
                                            Map user = {
                                              ...
                                              "role": result["data"]["role"] == "org_admin" ? "admin" : result["data"]["role"],
                                              "user_id": result["data"]["user"]["id"],
                                              "id": result["data"][
                                                  "${result["data"]["role"] == "org_admin" || result["data"]["role"] == "property_admin" || result["data"]["role"] == "branch_admin" ? "admin" : result["data"]["role"]}_id"],
                                              "belongs_to_id": result["data"]["role"] == "student"
                                                  ? result["data"]["Belongs_to"]["class_room_id"]
                                                  : result["data"]["role"] == "branch_admin"
                                                      ? result["data"]["Belongs_to"]["branch_id"]
                                                      : result["data"]["Belongs_to"]["property_id"],
                                              "class_room_id": result["data"]["Belongs_to"]["class_room_id"],
                                              "grade_id": result["data"]["Belongs_to"]["grade_id"],
                                            };
                                            if (widget.is_second_login == true) {
                                              prefs.setBool('is_user_2', false);
                                              await prefs.setString('user_2', jsonEncode(user));
                                            ...
                                            Navigator.of(context).pushAndRemoveUntil(
                                                createRoute(HomeScreen(
                                                  is_user_2: widget.is_second_login,
                                                )),
                                                (Route<dynamic> route) => false);
                                          } else {
                                            Navigator.of(context).pop();
                                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                              content: Text(translate(context).incorrect_username_password,
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                          content: Text(translate(context).enter_username_password,
                                              style:  TextStyle(fontFamily: getFontName(context))),
                                          duration: const Duration(seconds: 1),
                                        ));
                                      }
