...
class DropDown extends StatefulWidget {
  ...
  const DropDown(
      {Key? key, required this.words, required this.isMultiple, required this.selectedWords, this.isPagnet, this.getData, this.isNullable, this.type})
      : super(key: key);
...
class _DropDownState extends State<DropDown> {
  ...
  setUpdaData(words) {
    ...
    if (widget.selectedWords.isEmpty) {
      if (widget.isMultiple && widget.selectedWords.isEmpty && !allIntegers && widget.isPagnet == false ||
                ...
                child: Scrollbar(
                  ...
                  child: ListView(
                    ...
                    children: [
                            ...
                            if (widget.isMultiple)
                              GestureDetector(
                                child: CircleAvatar(
                                  ...
                                  radius: 15,
                                  child: Icon(
                                    selectedWords.length != widget.words.length ? Icons.check_circle_outline_outlined : Icons.check_circle_rounded,
                                    color: Colors.white,
                                    size: 20,
                                  ),
                                ),
                                onTap: () {
                                  if (selectedWords.length != widget.words.length) {
                                    selectedWords = [];
                                    selectedWords.addAll(widget.words.map((word) => word.toString()));
                                  } else {
                                    selectedWords = [];
                                  }
                                  setState(() {});
                      ...
                      if (isLodaing && widget.words.isEmpty) const CirculeProgress(),
                      if (!isLodaing && widget.words.isEmpty) Center(child: Text(translate(Get.context!).noDataAvailable)),
                      if (widget.words.isNotEmpty)
                        for (var word in searchDropController.text == ""
                            ? widget.words
                            : widget.words
                                .where((e) =>
                                    e.contains(searchDropController.text.toTitleCase()) ||
                                    e.contains(searchDropController.text) ||
                                    e.contains(searchDropController.text.toCapitalized()))
                                .toList())
                          Padding(
                            ...
                            child: InkWell(
                                ...
                                onTap: () {
                                  if (widget.isMultiple) {
                                    if (!selectedWords.contains(word.toString())) {
                                      setState(() {
...
class LongDropDown extends StatefulWidget {
  ...
  const LongDropDown({Key? key, required this.group, required this.selectedWords, this.isPagnet, this.getData, this.onAdd, this.onDelete})
      : super(key: key);
                              ...
                              if (value != null) {
                                ...
                                if (widget.selectedWords.contains("student")) {
                                  result = await ApiService().transferStudent({
                                    "studnet_id": widget.selectedWords[1]["id"],
                                    "new_property_id": item["id"].toString(),
                                    "new_class_id": int.parse(value.toString().split(" - ")[2]),
                                  }, jsonDecode(authService.user.toUser())["token"]);
                                } else if (widget.selectedWords.contains("teacher")) {
                                  result = await ApiService().transferTeacher({
                                    "teacher_id": widget.selectedWords[1]["id"],
                                    "new_property_id": item["id"].toString(),
                                    "new_class_id": int.parse(value.toString().split(" - ")[2]),
                                  }, jsonDecode(authService.user.toUser())["token"]);
                                }
...
Future<dynamic> dropDown(BuildContext context, List words, bool isMultiple, List selectedWords,
    {bool? isPagnet,
    String? type,
...
imageDropDown(BuildContext context, String title, Function() deleteOnTap, Function() galleryOnTap, Function() cameraOnTap) {
  return showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (BuildContext context) {
